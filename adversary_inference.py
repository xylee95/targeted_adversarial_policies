...
def main():
	...
	parser.add_argument('--save_dir', type=str, default='adv_inference',
	...
	if args.variant == 'A':
		print('Testing adversarial policy on nominal agent with action only')
		for i in range(10):
			_ = env.reset()
			...
			while not done and t < 1000: 
				...
				env_obs, env_r, done, info = env.step(adv_action)
		...
		print('Testing adversarial policy on nominal agent with state-action')
		for i in range(10):
			_ = env.reset()
			#reset to generate random adversarial goal
			adv_goal = env.goal_pos
	...
	save_path = args.save_dir + '/'  + args.env + '_' + args.variant + '_' + str(args.goals_norm) + '_' + str(args.seed)
